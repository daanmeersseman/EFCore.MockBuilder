...
using Bogus;
using EFCore.MockBuilder.Abstractions;
...
public class DbContextBuilderTests
{
    ...
    public void AddAllDataTypesEntity_WithCustomProperties_ShouldSetPropertiesCorrectly()
    {
        ...
        // Text Types
        Assert.Equal("RequiredValue", entity.RequiredString);
        Assert.Equal("MaxLengthString", entity.MaxLengthString);
        Assert.Equal("MinLen", entity.MinLengthString);
        Assert.Equal("StringLengthValue", entity.StringLengthString);
        ...
        Assert.Equal("123-456-7890", entity.PhoneNumber);
        Assert.Equal("https://www.example.com", entity.Url);
    }


    [Fact]
    public void APropertyWithCustomDataType_ShouldBeSetCorrectly_WhenAnExpressionIsProvided()
    {
        // Arrange
        var context = CreateInMemoryContext();

        var builder = new DbContextBuilder<DbContext>(context)
            .WithPropertySetter<ValueObject>((_, _) => ValueObject.Parse("ExpectedValue"));
        ...
        Assert.NotNull(user.CustomValue);
        Assert.NotEmpty(user.CustomValue.ToString());
        Assert.Equal("ExpectedValue", user.CustomValue.ToString());
    }

    private sealed class ValueObjectPropertySetter() : PropertySetter<ValueObject>((_, _) => ValueObject.Parse("ExpectedValue"));

    [Fact]
    public void APropertyWithCustomDataType_ShouldBeSetCorrectly_WhenAPropertySetterIsProvided()
    {
        // Arrange
        var context = CreateInMemoryContext();

        var builder = new DbContextBuilder<DbContext>(context)
            .WithPropertySetter<ValueObjectPropertySetter>();
    ...
    [Fact]
    public void APropertyWithCustomDataType_ShouldBeSetCorrectly_WhenAPropertySetterIsAddedToTheDefaultPropertySetters()
    {
        // Arrange
        var context = CreateInMemoryContext();

        DefaultPropertySetters.AddPropertySetter<ValueObjectPropertySetter>();
